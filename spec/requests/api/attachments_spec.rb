...
describe Attachment do 
  ...
  describe "When filter by admin tags" do
    ...
    it "allow attachment creation only with person valid admin tags" do
      ...
      expect(api_response.data.id).to eq(attachment.id.to_s)
      ...
      expect do
        api_create "/attachments",
          jsonapi_attachment('natural_docket_seeds', seed2.id.to_s, :png)
      end.to change { Attachment.count }.by(1)
    ...
    it "index attachment with admin user active tags" do
      ...
      expect(api_response.data[0].id).to eq(att4.id.to_s)
      expect(api_response.data[1].id).to eq(att3.id.to_s)
