...
describe IssueToken do
  ...
  context 'reply_observation endpoint' do
    it 'does not return observations when they are answered' do
      ...
      observations.each do |observation|
        api_update_issue_token(
          "/issue_tokens/#{issue_token.token}/reply_observation",
          observation: {
            data: {
              type: 'observations',
              id: observation.id,
    ...
    it 'replies and attaches file to an observation' do
      issue_token = IssueToken.where(issue: issue).first
      observations = issue_token.observations
      ...
      expect(seed.reload.attachments.count).to eq 2
    ...
    it 'replies and attaches multiple files to an observation' do
      issue_token = IssueToken.where(issue: issue).first
      observations = issue_token.observations
          ...
          attachments: [
            ...
            },
            {
              data: {
                ...
                attributes: {
                  document: "data:#{mime_for(:jpg)};base64,#{bytes_for('jpg')}",
                  document_file_name: 'attachment.jpg',
                  document_content_type: mime_for(:jpg)
                }
              }
            }
      ...
      expect(seed.reload.attachments.count).to eq 4
    ...
    it 'can not reply to an observation when token is invalid' do
      issue_token = IssueToken.where(issue: issue).first
    ...
    it 'can not reply to an observation not belonging to the issue token' do
      issue_token = IssueToken.where(issue: issue).first
